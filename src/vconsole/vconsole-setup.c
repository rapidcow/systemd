...
static void context_merge_config(
        ...
        context_merge(dst, src, src_compat, font_double);
}
...
static int context_read_creds(Context *c) {
        ...
        r = read_credential_strings_many(
                        ...
                        "vconsole.font_double",   &v.font_double);
        if (r < 0)
...
static int context_read_proc_cmdline(Context *c) {
        ...
        r = proc_cmdline_get_key_many(
                        ...
                        "vconsole.font.double",   &w.font_double);
        if (r < 0) {
                if (r != -ENOENT)
...
static int font_load_and_wait(const char *vc, Context *c) {
        ...
        const char
                ...
                *font_double = empty_to_null(c->font_double);
        ...
        if (!isempty(font_double) && strcmp("0", font_double) && strcmp("no", font_double))
                args[i++] = "-d";
        if (font)
                args[i++] = font;
        args[i++] = NULL;
